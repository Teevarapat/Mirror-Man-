...
class CheckCollision (object):
	...
	def iamhit(self, player, car, car2, car3, car4):
		...
		elif self.player.getX() > self.car2.getX() - 100 and self.player.getX() + width < self.car2.getX():
			if self.player.getY() + height > self.car2.getY() and self.player.getY() < self.car2.getY() + 50:
				print 'i am hit by car2'
				return True

		elif self.player.getX() > self.car4.getX()- 100 and self.player.getX() + width < self.car4.getX():
